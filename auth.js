/* ============================================================
   ...
   - Para notificar por correo cada registro se usa Formspree
     (https://formspree.io). Ya tienes tu formulario creado, así
     que esto ya está listo para funcionar tal cual.
 
   Formspree envía el aviso al correo con el que creaste ese
   formulario en formspree.io (revísalo en tu panel de Formspree).
   Si algún día quieres cambiar a qué correo llega, entra a tu
   cuenta de Formspree, crea/edita el formulario y reemplaza la
   URL de "endpoint" aquí abajo por la nueva.
   ============================================================ */
 
const FORMSPREE_CONFIG = {
  enabled: true,
  endpoint: "https://formspree.io/f/mdeoeavn"
};
...
/* ---------- notificación por correo (Formspree) ---------- */
async function notifyNewRegistration({ nombre, correo, telefono }) {
  if (!FORMSPREE_CONFIG.enabled) {
    console.info(
      ...
      "Completa FORMSPREE_CONFIG en js/auth.js para activarla."
    );
  ...
  try {
    const response = await fetch(FORMSPREE_CONFIG.endpoint, {
      ...
      body: JSON.stringify({
        _subject: "Nuevo registro en DeskAssist Global",
        Nombre: nombre,
        Correo: correo,
    ...
    if (!response.ok) {
      ...
      throw new Error(detail.error || "Formspree rechazó el envío");
    }
    return { sent: true };
  } catch (err) {
